...
{
    if(!inited) return;
    firebase::functions::HttpsCallableReference func = functions->GetHttpsCallable(function_name.utf8().ptr());
    firebase::Future<firebase::functions::HttpsCallableResult> future = func.Call();
    FunctionContext *fc = new FunctionContext(function_name.utf8().ptr(), this);
